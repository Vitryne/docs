...
# ADR-019 — Tokens de tema no Tailwind CSS v4 via @theme no CSS global
...
único para as cores da marca e as fontes (Poppins, Bai Jamjuree) usadas em login e cadastro.
...
- Compatível com as classes utilitárias já usadas nos componentes (`bg-primary`,
  `text-text-secondary`)
- Fácil de trocar quando o Design System oficial da marca chegar

## Alternativas consideradas

1. **Tokens declarados em `@theme` dentro de `src/styles/global.css`, sem
   `tailwind.config.ts`** — escolhida para registro, sujeita à validação.
...
- O valor de `--color-primary` (`#9530D9`) foi definido sem o Design System oficial da marca, então
  toda a interface pode precisar de reajuste quando os tokens definitivos chegarem
